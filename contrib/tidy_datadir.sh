...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Dash Black database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Dash Black datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Dash Black datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Dash Black 0.7 datadir."
    ;;
  3)
    echo "Detected Dash Black pre-0.8 datadir."
    ;;
  4)
    echo "Detected Dash Black 0.8 datadir."
    ;;
esac
